...
# 5. Ethernet (L1&L2 : Physical Layer&Data-link Layer)
...
#### 운영 체제(OS) 수준 DNS 캐싱
- OS 수준 DNS 확인자는 DNS 쿼리가 컴퓨터를 떠나기 전의 두 번째 중단점이며, 로컬에 있는 마지막 중단점   
- 이 쿼리를 처리하도록 설계된 OS 내부의 프로세스를 일반적으로 **"스텁 확인자"** 또는 **"DNS 클라이언트"** 라고 함   
...
- DNSKEY 레코드 :  DNSSEC(Domain Name System Security Extension) 서명을 확인하는 데 사용되는 공개 키가 포함되어 있습니다.   
...
## 9.1 보안 프로토콜
### 9.1.1 보안 프로토콜 종류
- SSH(Secure Shell) : Application Layer(L7)에 위치, 원격 로그인 시에 통신을 암호화   
- SSL(Secure Socker Layer) : Transport Layer(L4) ~ Application Layer(L7) 사이에 위치, 데이터를 암호화   
- IPSec(IP Security) : Network Layer(L3)의 부분, IP datagram의 인증이나 암호화를 수행하는 프로토콜의 총칭   
## 9.2 SSL Protocol : Secure Socket Layer
### 9.2.1 SSL 과정
<img width="600" alt="SSL과정" src="https://user-images.githubusercontent.com/49184890/128445414-931e53b2-9dea-49ac-9202-cbb94e2beee1.PNG">   
   
### 9.2.2 대칭키와 비대칭키
#### 대칭키
하나의 키를 이용하여 암호화/복호화를 동시에 진행한다.   
- 장점: 빠른 시간 내에 암호를 만들고 해독하는 것이 가능하다.   
- 단점: 열쇠가 되는 대칭키를 잃어버렸을 때 본문을 해킹당하게 되는 보안상 취약점을 지니고 있다.   
#### 비대칭키
두 개의 키를 가지고 암/복호화를 진행하는데, 암호화 및 복호화를 진행할 때 서로가 쌍이 되는 것이다.   
- 장점: A,B의 두 가지 키가 있다면 A로 암호화한 암호문은 B를 이용해서 해석이 가능하고 반대로 B로 암호화한 암호문은 A를 이용해서 해석이 가능하다. 이때 A를 이용해서 B를 유추하거나 하는 것은 불가능하므로 키를 하나를 잘 보존한다면 매우 안전하다.   
- 단점: 하지만 서로 다른 두 개의 키를 이용해 암/복호화를 진행하기 때문에 많은 시간이 소요된다.   

#### 대칭키 + 비대칭키 => SSL
SSL을 이용한 표준 암호화 방식은 위의 두 가지의 방식을 복합 적용한 것으로 대칭키 암호화/비대칭키 암호화를 둘 다 사용하게 된다.   
즉, 효율이 좋은 대칭키 암/복호화를 사용하지만 보안상 취약한 점을 보완하기 위해 클라이언트가 비대칭키 암호화 방식으로 서버가 제공한 공개키를 이용하여 대칭키를 암호화하여 서버에게 제공, 대칭키를 이용하여 암호화된 통신을 진행한다.   
