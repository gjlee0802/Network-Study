...
리눅스 커널 네트워킹 서브시스템은 두 개의 git 저장소로 관리된다. 패치와 RFC는 두 저장소 모두에 대해 netdev 메일링 리스트로 전송된다. 다음은 두 git 트리다.   
- net(http://git.kernel.org/?p=linux/kernel/git/davem/net.git) : 이미 주류 트리에 포함된 코드 수정
- net-next(http://git.kernel.org/?p=linux/kernel/git/davem/net-next.git) : 향후 커널 릴리스에 포함될 새로운 코드   

## 1.3 정리
- 네트워크 장치 구조체(`net_device`)와 소켓 버퍼 구조체(`sk_buff`)에 대해서 다뤘다. 이 둘은 네트워킹 서브시스템의 가장 기본적인 구조체다.
이러한 구조체를 구성하는 거의 모든 요소와 그것의 사용에 관해서는 위의 참고 링크 혹은 교재의 부록 A의 상세한 설명을 참고하자.   
- 커널 네트워킹 스택에서의 패킷 이동과 관련된 주제 : 라우팅 서브시스템 탐색, 단편화, 단편화 제거, 프로토콜 핸들러 등록 등.
- IPv4, IPv6, ICMP4, ICMP6, ARP, 이웃 탐색과 다른 서브시스템들은 후에 다룬다.   

# 2. 넷링크 소켓
> 참고 : Why and How to Use Netlink Socket https://www.linuxjournal.com/article/7356

넷링크 소켓 인터페이스는 리눅스 커널 2.2에서 AF_NETLINK 소켓으로 처음 도입됐다.   
넷링크 소켓 인터페이스는 사용자 공간 프로세스와 커널 간의 불편한 IOCTL 통신 방법에 대한 좀 더 유연한 대안으로 만들어졌다.   
IOCTL 핸들러는 커널에서 사용자 공간으로 비동기 메시지를 보낼 수 없지만, 넷링크 소켓은 비동기 메시지를 보낼 수 있다.   
IOCTL은 IOCTL 번호를 정의해야 한다는 복잡성이 있지만 넷링크의 동작 모델은 매우 간단하다.   
소켓 API를 이용해 사용자 공간에서 넷링크 소켓을 열고 등록한다. 그리고 넷링크 소켓은 커널 넷링크 소켓과의 양방향 통신을 처리하며, 이때 일반적으로 다양한 시스템 설정을 구성하는 메시지를 보내고 커널로부터 응답을 되돌려받는다.   

## 2.1 IOCTL과 넷링크 비교
- 커널 서비스는 Netlink를 통해 사용자 응용 프로그램에 직접 정보를 보낼 수 있지만 과거에는 상대적으로 비용이 많이 드는 작업인 IOCTL 함수로 커널을 명시적으로 폴링했습니다.
- Netlink 통신은 비동기이며, 각 측은 상대방이 메시지를 보낸 후 어느 시점에서 메시지를 수신합니다. IOCTL는 순전히 동기입니다 : "이봐 커널, 일어나! 난 당신이 지금 내 요청을 처리해야합니다! 잘라!"
- Netlink는 커널과 여러 사용자 공간 프로세스 간의 멀티캐스트 통신을 지원하며 IOCTL는 엄격하게 일대일입니다.
- Netlink 메시지는 여러 가지 이유로(예: 메모리 부족)로 손실될 수 있지만 IOCTL는 일반적으로 즉각적인 처리 특성으로 인해 더 신뢰할 수 있습니다.
...
# 7. 리눅스 이웃 서브시스템(ARP for IPv4, NDISC for IPv6)
